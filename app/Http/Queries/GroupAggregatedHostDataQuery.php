...
class GroupAggregatedHostDataQuery extends QueryBuilder
{
    private $withHostRelationshipConstraints = null;

    public function __construct()
    {
        /** @var Tag|QueryBuilder $this */
        $query = Tag::query()
            ->withType('host')
            ->containing(':')
            ->whereHas('hosts')
        ...
        $this
            ->allowedFilters([
                AllowedFilter::scope('group', 'whereTagNameBeginsWith'),
        ...
        $this->withHostRelationshipConstraints[] = function ($query) {
            $query
                ...
                ->groupBy(['hosts.id', 'pivot_taggable_type', 'pivot_taggable_id', 'pivot_tag_id']);
        };
